...
def scale_by_kron(
    ...
    def update_fn(updates: base.Updates, state: dict, params: base.Params = None):
        ...
        nu = otu.tree_update_moment(updates, state["nu"], 0.95, 2)
        nu_hat = otu.tree_bias_correction(nu, 0.95, count_inc)
        ...
        rms = jnp.sqrt(
            jnp.mean(
                jnp.stack(
                    [jnp.mean(jnp.square(g)) for g in jax.tree.leaves(precond_gs)]
                )
            )
        )
        jax.lax.cond(
            count_inc % 100 == 0,
            lambda: jax.debug.print("update RMS: {rms}", rms=rms),
            lambda: None,
        )
