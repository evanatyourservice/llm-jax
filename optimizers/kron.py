...
def scale_by_kron(
    ...
    Args:
        ...
        raw_grads_update: bool, whether to use raw grads for update instead of
            momentum, used only for schedule-free kron.
    ...
    def update_fn(updates: base.Updates, state: dict, params: base.Params = None):
        ...
        update_in = updates
        if raw_grads_update:
            update_in = momentum_updates
        precond_gs = [
            map_fn(s, partial(_precond_grad, exprs=exprs), Q, g)
            for s, exprs, Q, g in zip(scanned_layers_, expressions, Qs, update_in)
        ]
...
def kron(
    ...
    opt = [
        scale_by_kron(
            ...
            raw_grads_update=raw_grads_update,
        )
    ]
    if weight_decay > 0:
