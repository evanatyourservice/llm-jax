...
from optimizers.tearfree import sketchy
...
class SketchyTest(parameterized.TestCase):
    ...
    def _make_null_state(self, d, k) -> sketchy._AxisState:
        return sketchy._init(sketchy.Options(rank=k), jnp.zeros((d,))).sketches.axes[0]
    ...
    def _make_eye_state(self, d, eigs, tail, ndim) -> sketchy._AxisState:
        ...
        ie = jnp.where(mask, (state.tail + state.eigvals**2), 1.0) ** (-1 / (2 * ndim))
        ie *= mask
        state._replace(inv_eigvals=ie)
        return state

    def _no_decay_options(self, sketch_size, epsilon=0.0):
        return sketchy.Options(rank=sketch_size, second_moment_decay=1, epsilon=epsilon)
    ...
    def test_basic(self, ndim, decay, init, tail, last_axis):
        ...
        if updated.tail > 0:
            self.assertAlmostEqual(updated.tail ** (-1 / (2 * ndim)), updated.inv_tail)
        else:
