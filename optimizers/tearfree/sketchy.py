...
def _locate_path(
    ...
    for p in path:
        if not hasattr(p, "key") and not hasattr(p, "idx"):
            raise ValueError("no key or idx found")
        carry = carry[p.key] if hasattr(p, "key") else carry[p.idx]
    ...
    def _tensor_state(path: ..., param: jax.Array) -> _TensorState:
        ...
        for d in param.shape:
            ...
            if memory_alloc:
                ...
                logging.info(
                    "custom rank: %d rank allocated to %s", k, _path_to_key(path)
                )
            else:
                logging.warning(
...
def _update(
    ...
    return new_updates, _SketchyState(count=state.count + 1, sketches=new_sketches)
