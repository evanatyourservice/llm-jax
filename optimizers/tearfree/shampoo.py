...
class Options:
    ...
    Attributes:
      use_CASPR_variant: If true, uses the CASPR variant of Shampoo.
...
def _blocks_metadata(
    ...
    blocks_per_large_axis = [param_shape[i] // options.block_size for i in large_axes]
...
def _init(options: Options, params: optax.Params) -> _ShampooState:
    ...
    def make_blocks(path: ..., param: jax.Array) -> _AxesBlocks:
        ...
        if any(
            ...
            raise ValueError(
                "param {} shape ({}) has large dims indivisible by block size {}".format(
                    path, param.shape, options.block_size
                )
            )
...
def _update_block_stats(
    ...
    with jax.named_scope("ShampooStats"):
        for axis, cov in enumerate(block.stats):
            ...
            batched_tensordot = jax.vmap(dot_all, in_axes=meta.blocks_axis, out_axes=0)
...
def _precondition_blocks_shampoo(
    ...
    preconditioner_inputs = [
        blocks_axis_letter + o + c for c, o in zip(contraction_letters, output_letters)
    ]
...
def _precondition_blocks_caspr(
    update: jax.Array, blocks: _AxesBlocks, meta: _BlocksMetadata
) -> jax.Array:
    """Precondition blocked gradients."""
    it = _einsum_letters(meta)
    ...
    preconditioner_outputs = []
    for i, o in enumerate(output_letters):
        p_o = list(blocked_input)
        p_o[i + 1] = o
        preconditioner_outputs.append("".join(p_o))

    formulas = [
        ",".join([blocked_input] + [p_in]) + "->" + p_out
        for p_in, p_out in zip(preconditioner_inputs, preconditioner_outputs)
    ]
    ...
    with jax.named_scope("PreconditionCASPR"):
        ...
        for i in range(2):
            ...
            for f, p in zip(formulas, preconditioners):
                to_sum.append(jnp.einsum(f, update, p))
            update = sum(to_sum)
        return update


def _split_exclusively(ls: Sequence[int], splits: Sequence[int]) -> list[list[int]]:
