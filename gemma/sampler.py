...
def _compute_attention_masks(
    ...
    input_mask = (
        jnp.zeros((bsz, seq_len), dtype=jnp.bool_).at[:, :max_seq_len].set(input_mask)
    )
...
class Sampler:
    ...
    def _sample_step(self, params, sampler_state: _SamplingState) -> _SamplingState:
    ...
    def __call__(
        ...
        sampling_state = self._compiled_sample_fn(self.params, initial_sampling_state)
